__all__ = ["AircraftModelParser", "scrape"]
...
from libs.models.aircraft_model import RawAircraftModel


class AircraftModelParser(Logging):
    ...
    def parse(self) -> RawAircraftModel:
        html = self.__download()
        soup = BeautifulSoup(html, "html.parser")

        result = RawAircraftModel()
...
def scrape(icao_list: List[str]):
    for icao in icao_list:
        parser = AircraftModelParser(icao)
        yield parser.parse()
